 ...
 *              otherwise the metadata note carries an `e` tag pointing at a separate
 *              Kind-1 content event whose `.content` field is the raw HTML/JS source.
 ...
 * Existing serialised events that carry `isGeneratedApp: true` (without `hosting`) are
 * treated as `hosting: 'nostr'` by the loaders.
 */
export type AppHosting = 'nostr' | 'url';
